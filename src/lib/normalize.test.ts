...
describe('normalizeFieldPair', () => {
  it('treats empty array fields as equivalent to undefined', () => {
    expect(normalizeFieldPair('mcp_servers', [], undefined)).toEqual([
      undefined,
      undefined,
    ]);
  });
  ...
  it('treats empty metadata objects as equivalent to undefined', () => {
    expect(normalizeFieldPair('metadata', {}, undefined)).toEqual([
      undefined,
      undefined,
    ]);
  });
  ...
  it('fills tool config entries from default_config before comparison', () => {
    ...
    expect(normalizeFieldPair('tools', local, remote)).toEqual([
      remote,
      remote,
    ]);
  });
  ...
  it('fills omitted multiagent agent versions from matching remote entries', () => {
    ...
    expect(normalizeFieldPair('multiagent', local, remote)).toEqual([
      remote,
      remote,
    ]);
  });
  ...
  it('fills omitted custom skill versions from matching remote entries', () => {
    ...
    const remote = [
      { type: 'custom', skill_id: 'skill_123', version: 'v3' },
    ];

    expect(normalizeFieldPair('skills', local, remote)).toEqual([
      remote,
      remote,
    ]);
  });
